...
node *restore_bptree_root(unsigned long zone_id)
{
    unsigned long node_id = get_bptree_rootid(zone_id);
    if(node_id != -1)
    {
...
void *find(node *root, char *key)
{
    ...
    if (leaf == NULL)
        return NULL;
    tree = leaf->tree_root;
    for (i = 0; i < leaf->num_keys && tree->key_cmp(leaf->keys[i], key) != 0; i++)
        ;
    if (i == leaf->num_keys)
        return NULL;
...
node *find_leaf(node *root, char *key)
{
    ...
    nd = root;
    while (!nd->is_leaf){
        for (i = 0; i < nd->num_keys && tree->key_cmp(nd->keys[i], key) <= 0; i++)
            ;
...
node *make_new_node(tree_root *tree)
{
    ...
    if(tree->save)
    {
        ...
        block->node_id = tree->alloc_block_func();
        if(block->node_id == -1)
        {
            free(nd);
            free(block);
...
#if 0
void save_leaf_node_block(node *nd)
{
    tree_root *tree = nd->tree_root;
    int i = 0;
    if(tree->save && nd->is_leaf)
    {
        node_block *block = nd->block;
        block->num_keys = nd->num_keys;
        block->is_leaf = nd->is_leaf;
        for(i = 0 ; i < nd->num_keys ; i++)
        {
            block->pointers_id[i] = tree->get_data_id(nd->pointers[i]);
            block->keys_id[i] = tree->get_key_id(nd->pointers[i]);
        }        
        block->parent_id = nd->parent->block->node_id;
    }
}
#endif
...
{
    node *root;
    tree_root *tree = left->tree_root;
    root = make_new_node(tree);
